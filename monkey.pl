%start search: width_search(state(door,floor,window,no),state(_,_,_,yes)).
...
print_station(state(A,B,C,D)) :- 
  write('обезьяна стоит '),
  (A=door,write(' у двери '); A=middle, write('в середине комнаты '); A=window,write('у окна ')),
  (B=floor,write('на полу, '); B=box, write('на ящике, ')),
  (B=floor,(C=door,write('ящик стоит у двери, '); C=middle, write('ящик стоит в середине комнаты, '); C=window,write('ящик стоит у окна, ')); B=box),
  (D=yes,write('уже взяла банан');D=no,write('ещё не взяла банан')),nl.
